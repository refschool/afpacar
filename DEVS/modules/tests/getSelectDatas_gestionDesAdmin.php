...
* @version v1.0
*/
class getSelectDatas_gestionDesAdmin extends TestCase
{
    /**
    * 
...
public function should_contains_all_avis()
{
    ...
    $sql= "SELECT user.id_user FROM user INNER JOIN role  ON user.id_role = role.id_role WHERE (role.name_role = 'ROLE_SUPER_ADMIN' OR role.name_role = 'ROLE_ADMIN') AND (user.is_active_user = 0 OR user.is_active_user = 1)";
    ...
    $spathSQL = $oInit->GLOBALS_INI["PATH_HOME"] . $oInit->GLOBALS_INI["PATH_MODEL"] . "liste_users_admin.sql";
